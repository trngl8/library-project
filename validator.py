...
class Validator():
    ...
    def validate(self, object):
        if isinstance(object, Book):
            if len(self.rules) == 0:
                return True
            if isinstance(object.isbn, self.rules["ISBN"]["type"]) and re.match(self.rules["ISBN"]["regexp"], object.isbn) and\
    isinstance(object.year, self.rules["year"]["type"]) and self.rules["year"]["min"]< object.year < self.rules["year"]["max"]:
                return True
            return False
        else:
            if len(object) == 0:
